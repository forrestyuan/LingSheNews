  ...
  $.ajax({
        ...
        success: function (data) {
            if (data.resultCode == "1") {
                ...
                +""
                +"<span></span>一 特别提示<br>"
                +"<span></span>澎湃新闻网（http://www.thepaper.cn）同意按照本协议的规定及其不时发布的操作规则提供基于互联网以及移动网的相关服务（以下称\"网络服务\"），为获得网络服务，服务使用人（以下称\"用户\"）应当同意本协议的全部条款并按照页面上的提示完成全部的注册程序。用户在进行注册程序过程中点击\"同意\"按钮即表示用户完全接受本协议项下的全部条款。<br>"
...
var WWW_LOGIN_COOKIE = "PAPER_WWW_LOGIN";
